...
JOIN projects ON pledges.project_id = projects.id GROUP BY title HAVING SUM(pledges.amount) >= projects.funding_goal"
end

def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_amount_and_users_name
"SELECT users.name, SUM(pledges.amount) FROM pledges
JOIN users ON pledges.user_id = users.id GROUP BY name ORDER BY SUM(pledges.amount)"
end
